...
from sponge import * ; from xHeaders import *
            ...
            while True:      
                try:
                    ...
                    if 'besto_love' in self.input_msg[:10]:
                        self.CliEnts.send(xSEndMsg(f'''[b][c][FFD700]━━━━━━━━━━━━[b][c][{ArA_CoLor()}]\n  مدعوم بواسطة: فريق HIMO BOT  \n[b][c][FFD700]━━━━━━━━━━━━[b][c][{ArA_CoLor()}]Expire In => [ffffff]{self.Expire}\n[C][B][FFFFFF]لعرض الاوامر اكتب[b][c][{ArA_CoLor()}]/[C][B]help\n[C][B][ffffff]WELCOME TO BOT \n[b][c][{ArA_CoLor()}]BOT™ T_MAX® BOT1''', 2 , self.DeCode_CliEnt_Uid , self.DeCode_CliEnt_Uid , key , iv))
                        time.sleep(0.3)
                        self.CliEnts.close() ; self.CliEnts2.close()
                        self.Connect_SerVer(Token , tok , host , port , key , iv , host2 , port2)	                    	 	 
                                                               
                    if b'/help' in self.DaTa or b'/help' in self.DaTa or 'tv' in self.input_msg[:2]:
...
سبام رومات:
[C][B][ffffff]/[C][B]sp/ [ id ][b][c][{ArA_CoLor()}]
معرفة حالة اللاعب:
[C][B][ffffff]/[C][B]st/ [ id ]
زيادة عدد زوار الحساب:
[C][B][ffffff]/[C][B]visit  [ id ][b][c][{ArA_CoLor()}]
تزويد 100 لايك للحساب 
[C][B][ffffff]/[C][B]like  [ id ]''', 2 , self.DeCode_CliEnt_Uid , self.DeCode_CliEnt_Uid , key , iv))
                            time.sleep(0.3)
...
[C][B][{ArA_CoLor()}]@T_max530_vip
[C][B][FFFFFF]━━━━━━━━━━━━''', 2 , self.DeCode_CliEnt_Uid , self.DeCode_CliEnt_Uid , key , iv))
                            time.sleep(0.3)                            
                            self.CliEnts.close() ; self.CliEnts2.close()
                            self.Connect_SerVer(Token , tok , host , port , key , iv , host2 , port2)	   	       		
                        elif False == self.result:
                            DeLet_Uid(self.DeCode_CliEnt_Uid , Token)  
                    if b'/shop' in self.DaTa or b'/shop' in self.DaTa or 'tv' in self.input_msg[:2]:
                        self.result = ChEck_The_Uid(self.DeCode_CliEnt_Uid)
                        if self.result:
                            self.Status , self.Expire = self.result
                            self.CliEnts.send(xSEndMsg(f'''[b][c][FFD700]━━━━━━━━━━━━
[b][c][{ArA_CoLor()}]BOT CLAN CHAT
[C][B][FFD700]━━━━━━━━━━━━
[b][c][{ArA_CoLor()}]BOT FRIEND
[b][c][FFD700]━━━━━━━━━━━━
[b][c][{ArA_CoLor()}]BOT LEVEL
[b][c][FFD700]━━━━━━━━━━━━
[b][c][{ArA_CoLor()}]VPS
[b][c][FFD700]━━━━━━━━━━━━
[b][c][{ArA_CoLor()}]BOT GLORY
[b][c][FFD700]━━━━━━━━━━━━
    \n''', 2 , self.DeCode_CliEnt_Uid , self.DeCode_CliEnt_Uid , key , iv))
                            time.sleep(0.3)		
                            self.CliEnts.send(xSEndMsg(f'''
...
[C][B][FFFFFF]instagram:
[C][B][{ArA_CoLor()}]=>  T_max530_vip
[b][c][FFD700]━━━━━━━━━━━━''', 2 , self.DeCode_CliEnt_Uid , self.DeCode_CliEnt_Uid , key , iv))
                            time.sleep(0.3)
                            self.CliEnts.send(xSEndMsg(f'''
    [b][c][FFD700]━━━━━━━━━━━━[ffffff]
    [C][B][FFFFFF]TIKTOK:
    [b][c][{ArA_CoLor()}]=> b_13_ff_8
    [FFD700]━━━━━━━━━━━━[ffffff]\n''', 2 , self.DeCode_CliEnt_Uid , self.DeCode_CliEnt_Uid , key , iv))
                            time.sleep(0.3)                                                        
                            self.CliEnts.close() ; self.CliEnts2.close()
                            self.Connect_SerVer(Token , tok , host , port , key , iv , host2 , port2)	   	       		
                            ...
                            DeLet_Uid(self.DeCode_CliEnt_Uid , Token)   
                    if b'/new' in self.DaTa or b'/new' in self.DaTa or 'tv' in self.input_msg[:2]:
                        self.result = ChEck_The_Uid(self.DeCode_CliEnt_Uid)
                        if self.result:
                            self.Status , self.Expire = self.result
                            self.CliEnts.send(xSEndMsg(f'''ترقب يصدقي الميزات الجديدة
[C][B][{ArA_CoLor()}]=> /crazy
[C][B][{ArA_CoLor()}]=> /addlike
[C][B][{ArA_CoLor()}]=> /bio
[C][B][{ArA_CoLor()}]=> /name
[C][B][{ArA_CoLor()}]=> /game''', 2 , self.DeCode_CliEnt_Uid , self.DeCode_CliEnt_Uid , key , iv))
                            time.sleep(0.3)                                                        
                            self.CliEnts.close() ; self.CliEnts2.close()
                            self.Connect_SerVer(Token , tok , host , port , key , iv , host2 , port2)	   	       		
                        elif False == self.result:
                        ...
                        self.id , self.nm = (self.input_msg[6:].split(" ", 1) if " " in self.input_msg[6:] else [self.input_msg[6:], "BOT™ T_MAX®"])  
                        ...
                        self.ChEck_ReGister = ChEck_The_Uid(self.DeCode_CliEnt_Uid)
                        self.id , self.nm = (self.input_msg[5:].split(" ", 1) if " " in self.input_msg[5:] else [self.input_msg[5:], f'\n[b][c][{ArA_CoLor()}]لشراء بوت توصل مع\n[C][B][FFFFFF]━━━━━━━━━━━━\n[C][B][FFFFFF]instagram:\n[C][B][{ArA_CoLor()}]T_max530_vip \n[C][B][{ArA_CoLor()}]@VIP_T_MAX\n[C][B][FFFFFF]━━━━━━━━━━━━'])
                        self.Zx = ChEck_Commande(self.id)
                        if self.ChEck_ReGister and True == self.Zx:
                        ...
                        self.Zx = ChEck_Commande(self.id)
                        if self.ChEck_ReGister and True == self.Zx:
                            self.CliEnts.send(xSEndMsg(f'\n[b][c][{ArA_CoLor()}] Wait for tchitchi msg : {self.id}\n', 2 , self.DeCode_CliEnt_Uid , self.DeCode_CliEnt_Uid , key , iv))
          ...
          RemaininG : {self.res}
           ...
           [90EE90]Dev : himo.bot1 Team \n''', 2 , self.DeCode_CliEnt_Uid , self.DeCode_CliEnt_Uid , key , iv))
                            ...
                            if self.ChEck_ReGister:         	  
                                self.CliEnts.send(xSEndMsg(f'\n[b][c][{ArA_CoLor()}] 5 In Squid To {xMsGFixinG(self.id)}\n[FF6F61]D[FF8A65]e[FFB74D]v[FFD54F]e[FFF176]l[AED581]o[81C784]p[4DB6AC]e[4FC3F7]d[7986CB] h[BA68C8]i[F06292]m[FF6F61]o[FF8A65]b[FFB74D]o[FFD54F] H[FFF176]I[AED581]M[81C784]O[4DB6AC]BOT[4FC3F7]I\n', 2 , self.DeCode_CliEnt_Uid , self.DeCode_CliEnt_Uid , key , iv))
                            ...
                            self.ChEck_ReGister = ChEck_The_Uid(self.DeCode_CliEnt_Uid)
                            if self.ChEck_ReGister:         	  
                                self.CliEnts.send(xSEndMsg(f'\n[b][c][{ArA_CoLor()}] 6 In Squid To{xMsGFixinG(self.id)}\n[FF6F61]D[FF8A65]e[FFB74D]v[FFD54F]e[FFF176]l[AED581]o[81C784]p[4DB6AC]e[4FC3F7]d[7986CB] h[BA68C8]i[F06292]O[FF6F61]m[FF8A65]b[FFB74D]o[FFD54F] H[FFF176]I[AED581]M[81C784]O[4DB6AC]B[4FC3F7]O\n', 2 , self.DeCode_CliEnt_Uid , self.DeCode_CliEnt_Uid , key , iv))
                                self.CliEnts2.send(OpEnSq(key , iv))
                                time.sleep(0.5)
                            ...
                            self.ChEck_ReGister = ChEck_The_Uid(self.DeCode_CliEnt_Uid)
                            if self.ChEck_ReGister:         	  
                                self.CliEnts.send(xSEndMsg(f'\n[b][c][{ArA_CoLor()}]3 In Squid To {xMsGFixinG(self.id)}\n[FF6F61]D[FF8A65]e[FFB74D]v[FFD54F]e[FFF176]l[AED581]o[81C784]p[4DB6AC]e[4FC3F7]d[7986CB] H[BA68C8]I[F06292]M[FF6F61]O[FF8A65]B[FFB74D]OT[FFD54F] H[FFF176]I[AED581]M[81C784]O[4DB6AC]B[4FC3F7]OT\n', 2 , self.DeCode_CliEnt_Uid , self.DeCode_CliEnt_Uid , key , iv))
                                self.CliEnts2.send(OpEnSq(key , iv))
                                time.sleep(0.5)
                            ...
                            self.Zx = ChEck_Commande(self.id)
                            if self.ChEck_ReGister and True == self.Zx:  
                                self.CliEnts.send(xSEndMsg(f'\n[b][c][{ArA_CoLor()}] 3 In Squid To {xMsGFixinG(self.id)}\n[FF6F61]D[FF8A65]e[FFB74D]v[FFD54F]e[FFF176]l[AED581]o[81C784]p[4DB6AC]e[4FC3F7]d[7986CB] S[BA68C8]P[F06292]O[FF6F61]N[FF8A65]G[FFB74D]E[FFD54F] H[FFF176]I[AED581]M[81C784]O[4DB6AC]BO[4FC3F7]T\n', 2 , self.DeCode_CliEnt_Uid , self.DeCode_CliEnt_Uid , key , iv))
                                self.CliEnts2.send(OpEnSq(key , iv))
                                time.sleep(0.5)
                                self.CliEnts2.send(cHSq(3 , self.DeCode_CliEnt_Uid , key , iv))
                                time.sleep(0.5)
                            ...
                            self.Zx = ChEck_Commande(self.id)    
                            if self.ChEck_ReGister and True == self.Zx:  
                                self.CliEnts.send(xSEndMsg(f'\n[b][c][{ArA_CoLor()}] GeTinG PLayer {xMsGFixinG(self.id)}\n[FF6F61]D[FF8A65]e[FFB74D]v[FFD54F]e[FFF176]l[AED581]o[81C784]p[4DB6AC]e[4FC3F7]d[7986CB] S[BA68C8]P[F06292]O[FF6F61]N[FF8A65]G[FFB74D]E[FFD54F] H[FFF176]I[AED581]M[81C784]O[4DB6AC]BOT[4FC3F7]I', 2 , self.DeCode_CliEnt_Uid , self.DeCode_CliEnt_Uid , key , iv))
                                self.CliEnts2.send(OpEnSq(key , iv))
                                time.sleep(0.5)
                                self.CliEnts2.send(cHSq(5 , self.DeCode_CliEnt_Uid , key , iv))
                                time.sleep(0.5)			         
                    ...
                    if '/x/' in self.input_msg[:4]:
                        ...
                        self.id , self.nm = (self.input_msg[4:].split(" ", 1) if " " in self.input_msg[4:] else [self.input_msg[4:], "b_13_ff_8"])
                        self.Zx = ChEck_Commande(self.id)	
                        ...
                        self.id = parts[0] 
                        self.ghost_name = parts[1] if len(parts) > 1 else "TikTok:b_13_ff_8"
                        self.Zx = ChEck_Commande(self.id)
                        if True == self.Zx:
                            try:
...
def StarT_SerVer():
    ...
    FF_CLient('4095286181','66A4389B35B7F9AAA2A80F67B9F7750DAB9E0437E47416A1FABAC39B05617D37)
